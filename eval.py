...
parser.add_argument("--data-path", type=str, default='/mnt/sdb1/data/widerface/train/cls_data')
#--------------------model options--------------------
parser.add_argument("--start-switch", type=str, default=None)
parser.add_argument("--model", type=str, default="MobileFaceNetUltraLite")
parser.add_argument("--model-restore", type=str, default="/mnt/sdb3/Git_mess/mask_on_face_cls/checkpoints/best.pth")
...
if args.with_arc == False:
    ...
    if args.model_restore is not None:
        if os.path.exists(args.model_restore):
            ...
            Net = torch.load(args.model_restore, map_location=torch.device('cpu'))
    ...
    for epoch in range(args.start_epoch, args.end_epoch):
        ...
        print(classes_total[0], classes_total[1], total)
# # Train with arcloss
# else:
#     if args.model == "resnet18" :
#         Net = models.resnet18(num_classes=1000, pretrained=True)
